...
function startGame() {
    ...
    //Building ID
    userId = gameRounds[currentRound].userNameF + "." + gameRounds[currentRound].userNameL + "@wiesbaden.de"; //Building ID
    ...
    //count and Display Amount of Questions
    let questionAmount = questions.length;
    document.getElementById('questionAmount').textContent = questionAmount;
...
function updateTable () {
    ...
    //Table Data
        ...
        document.getElementById('userAdress').textContent = gameRounds[currentRound].userId;
        ...
        document.getElementById('userScore').textContent = gameRounds[currentRound].score;
...
function sendRoundsToServer() {
    // 1. Hole die letzte Runde
  const lastRound = gameRounds[gameRounds.length - 1];
  if (!lastRound) {
    console.warn("Keine Runde zum Senden vorhanden");
    return;
  }
  ...
  const roundData = {
    ...
    userId: lastRound.userId,
    score: lastRound.score,
    roundStatus: lastRound.roundStatus,
    documentation: lastRound.documentation
  };

  console.log("Sende nur die letzte Runde an den Server:", roundData);
  ...
  .then(res => res.text())
  .then(msg => {
    console.log(`Runde ${lastRound.roundNumber} gespeichert:`, msg);
  })
  .catch(err => {
    console.error('Fehler beim Speichern der letzten Runde:', err);
  });
}
...
function countQuestion () {
    ...
    document.getElementById('currentQuestion').textContent = displayQuestion; //Display the current Question
}
...
function nextQuestion() {
    ...
    if (currentQuestion < questions.length) { //if there are Questions left..
        ...
        alert("Super - alle Fragen beantwortet - hier geht es zur Auswertung :)")
